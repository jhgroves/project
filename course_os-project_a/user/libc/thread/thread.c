...
int thread_create(thread_t *thread, void *(*func)(void*), void *arg)
{
	if(thread == NULL)
	{
		int ret = __syscall3(SYSCALL_THREAD_CHECK,(long) thread, (long)func, (long)arg);
		if(ret == NULL)
			return -2;
		return -3;
	}
	...
	{
		int ret = __syscall3(SYSCALL_THREAD_CREATE,(long) thread, (long)func, (long)arg);
		if(ret == -5)
			return -2;
		return ret;
	}
...
{
	return __syscall0(SYSCALL_THREAD_SELF);
}
/*
...
{
	__syscall0(SYSCALL_THREAD_REMOVE);
}
...
{
	printf("syscall happening");
	int test = __syscall2(SYSCALL_THREAD_JOIN,(long) thread, (long)result);
	if(test == -5)
		return -3;
	return test;
...
{
	return __syscall1(SYSCALL_THREAD_TID, (long)thread);
}
