...
uint32_t __sched_remove_task(sched_task * task) {
    ...
    switch (task->state) {
        ...
        case TASK_STATE_ACTIVE: {
            ...
            if (IS_PROCESS(task)) {
                process_destroy(AS_PROCESS(task)->PID);
    ...
    switch (last_task->state) {
        case TASK_STATE_INACTIVE: {
            ...
            if (IS_PROCESS(active_task)) {
            	...
            	os_printf("ABOUT TO LOAD THREAD STATE ----------------\n");
                //AS_KTHREAD(active_task)->cb_handler();
                load_kthread_process_state(AS_KTHREAD(active_task));
            }
        ...
        case TASK_STATE_ACTIVE: {
            if (prq_count(active_tasks) > 1) {
                ...
                if (IS_PROCESS(active_task)) {
                    ...
                    os_printf("SAVING THREAD STATE ----------------------------------\n");
                    // FIXME: implement
                    active_task->state = 3;
                    save_kthread_process_state(AS_KTHREAD(active_task));
                }
                ...
                if (IS_PROCESS(active_task)){
                	...
                	os_printf("ABOUT TO LOAD PROCESS STATE ----------------\n");
                    load_process_state(AS_PROCESS(active_task)); // continue with the next process
                } else if (IS_KTHREAD(active_task)) {
                    __sched_emit_messages();
                	os_printf("task state active ABOUT TO LOAD THREAD STATE ----------------\n");
                    // FIXME: implement
                	//vm_enable_vas(AS_KTHREAD(active_task)->parentProcess->stored_vas);
                    __sched_resume_timer_irq();
                    load_kthread_process_state(AS_KTHREAD(active_task));
                }
            }
...
uint32_t sched_add_task(sched_task * task) {
    if (task) {
        ...
        if(IS_KTHREAD(task))
        {
        	hmap_put(cb_map, ((long)((kthread_handle*)task->task)->cb_handler), (void *)(((kthread_handle*)task->task)->TID));
        }
        return task->tid;
    }
...
long get_kthread_from_map(long cb_handler)
{
	long task1 = (long)(hmap_get(cb_map,cb_handler));
	return task1;
}
